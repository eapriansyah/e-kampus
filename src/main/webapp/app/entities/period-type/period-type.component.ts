...
export class PeriodTypeComponent implements OnInit, OnDestroy {
    ...
    transition() {
        this.router.navigate(['/period-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/period-type', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    sort() {
        ...
        if (this.predicate !== 'idPeriodType') {
            result.push('idPeriodType');
        }
        return result;
    }
    ...
    executeProcess(data) {
        this.periodTypeService.executeProcess(data).subscribe(
    ...
    updateRowData(event) {
        if (event.data.id !== undefined) {
            this.periodTypeService.update(event.data)
                .subscribe((res: PeriodType) =>
                    this.onRowDataSaveSuccess(res), (res: Response) => this.onRowDataSaveError(res));
        } else {
            this.periodTypeService.create(event.data)
                .subscribe((res: PeriodType) =>
                    this.onRowDataSaveSuccess(res), (res: Response) => this.onRowDataSaveError(res));
        }
    }

    private onRowDataSaveSuccess(result: PeriodType) {
        this.toasterService.showToaster('info', 'PeriodType Saved', 'Data saved..');
    }
