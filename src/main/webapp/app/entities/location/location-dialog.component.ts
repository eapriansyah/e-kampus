...
export class LocationDialogComponent implements OnInit {
    ...
    save() {
        this.isSaving = true;
        if (this.location.idGeoBoundary !== undefined) {
    ...
    private onSaveSuccess(result: Location, isCreated: boolean) {
        this.alertService.success(
            isCreated ? 'kampusApp.location.created'
            : 'kampusApp.location.updated',
            { param : result.idGeoBoundary }, null);

        this.toaster.showToaster('info', 'Data Save', 'Data Location di simpan !');
